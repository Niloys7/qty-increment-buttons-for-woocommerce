<?php
/*
 * Plugin Name: Qty Plus Minus Buttons for WooCommerce
...
class qib_settings {
	...
	public function __construct() {
		...
		$this->settings_list = array(
			...
			'qib_archive_display' => array(
				...
				'options' => array( 'None', 'Shop', 'Category', 'Shop & Category' ),
				'tip'     => true,
			),

			'qib_archive_after'   => array(
				...
				'options' => array( 'Before Add to cart', 'After Add to cart' ),
				'arg'     => ( is_bool( get_option( 'qib_settingz' ) ) || get_option( 'qib_settingz' )['qib_archive_display'] == 'None' ) ? array( 'class' => 'hidden' ) : null,
				'tip'     => true,
			),

			'qib_auto_table'      => array(
				'val'   => false,
			...
			'qib_button_style'    => array(
				...
				'options' => array( 'Black', 'Blue', 'Brown', 'Orange', 'Red', 'Silver' ),
				'tip'     => true,
			),

			'qib_button_height'   => array(
				'val'   => 35,
				'title' => 'Button height',
				'type'  => 'number',
				'tab'   => 'configuration',
				'descr' => 'Recommended 25-40 pixels. Quantity input field and Add to cart button will have the same height.',
				'arg'   => array( 'class' => 'qib_sizes' ),
			),

			'qib_button_width'    => array(
				'val'   => 30,
				'title' => 'Button width',
				'type'  => 'number',
				'tab'   => 'configuration',
	...
	public function display_options() {
		...
		register_setting( 'qib_settingz', 'qib_settingz', array( $this, 'sanitize' ) );
		// ID / title / cb / page
		add_settings_section( 'configuration_section', null, array( $this, 'print_section_info' ), $this->settings_page_name );
		...
		foreach ( $arr as $key => $item ) {
			if ( $active_tab == $arr[ $key ]['tab'] ) {
				...
				add_settings_field(
					...
					array( $this, 'qib_print_field' ), // Callback
	...
	public function admin_enqueue_scripts( $page ) {
		...
		wp_enqueue_script( $plugin_short_slug . '-admin', plugins_url( '/js/' . $plugin_slug . '-admin' . '.js', __FILE__ ), array( 'jquery' ), '', true );
	...
	public function public_enqueue_scripts() {
		...
		wp_enqueue_script( $plugin_short_slug . '-public', plugins_url( '/js/public.js', __FILE__ ), array( 'jquery' ), '', true );
	}
	/**
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || ( get_site_option( 'active_sitewide_plugins' ) && array_key_exists( 'woocommerce/woocommerce.php', get_site_option( 'active_sitewide_plugins' ) ) ) ) {
	...
	function qty_increment_buttons( $args ) {
		...
		if ( $args['archive_display'] == 'Shop' && is_product_category() ) {
			return;
		}
		if ( $args['archive_display'] == 'Category' && is_shop() ) {
			return;
		}
		...
		} elseif ( $theme == 'Astra' || $parent_theme == 'Astra' ) {
